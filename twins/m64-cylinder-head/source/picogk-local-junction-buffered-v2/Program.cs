...
if(args.Length!=4 || args[0]!="--witness" ||
   !float.TryParse(args[3],NumberStyles.Float,CultureInfo.InvariantCulture,out float h) || (h!=0.2f&&h!=0.1f))
{ Console.Error.WriteLine("Usage: BufferedJunctionV2 --witness CRITERIA NEW_DIR 0.2_OR_0.1"); return 2; }
...
if(p.GetProperty("voxel_world_units").GetSingle()!=h ||
   ...
   p.GetProperty("fixed_interior_margin_world_units").GetSingle()!=0.6f ||
...
try
{
    ...
    float margin=p.GetProperty("fixed_interior_margin_world_units").GetSingle();
    ...
    var report=new{schema="m64-picogk-local-junction-buffered-witness/v2",
        ...
        margin_voxels=margin/h,margin_world_units=margin,margin_mode="fixed_world_for_resolution_comparison",margin_is_not_extraction_proof=true,
        ...
        source_master_modified=false,private_head_processed=false,finer_resolution_executed=h==0.1f,
