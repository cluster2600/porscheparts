...
def declared_screen(native, rows, roi):
    ...
    restored = len(vdb) == 6 and all(row['exact_value_comparison_pass'] for row in vdb.values())
    normalized = all(row['normalized']['topology']['closed_oriented_combinatorial_surface_screen_pass']
                     for row in rows.values())
    contained = roi['all_changed_triangle_supports_inside_convex_ROI']
    return bool(signs and changed and restored and normalized and contained)


def run(directory, output):
    topo, direct = load_helpers()
    start = time.monotonic()
    policy_path = Path(__file__).with_name('criteria.json')
    ...
    if (native['schema'] != 'm64-picogk-local-junction-buffered-witness/v1' or
            native['voxel_world_units'] != .2 or native['private_head_processed'] is not False or
            native['policy_sha256'] != source_hashes[policy_path.name] or
            native['authorized_ROI_low'] != [-8, -3, -8] or native['authorized_ROI_high'] != [8, 3, 8]):
        raise ValueError('Only this policy-bound 0.2 buffered synthetic witness is accepted')
    ...
    report = {
        'schema': 'm64-picogk-buffered-surface-audit/v1', 'source_sha256': source_hashes,
        'native_report_sha256': native_hash, 'native_VDB_sha256': topo.sha(vdb),
        'native_occupancy_status_retained': native['status'],
        'numpy_version': np.__version__, 'meshes': rows, 'normalized_mesh_ROI_check': roi,
        ...
        'second_resolution_or_private_head_authorized': False,
