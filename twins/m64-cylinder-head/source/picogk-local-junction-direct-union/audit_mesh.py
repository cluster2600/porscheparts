...
def topology(mesh):
    ...
    return {
        ...
        'surface_components_including_degenerate_faces': len(welded.split(only_watertight=False, repair=False)),
...
def run(directory, output):
    ...
    if sha(vdb) != native['native_VDB']['sha256']:
        raise ValueError('VDB does not match native report')
    rows = {}
    ...
    report = {
        'schema': 'm64-local-junction-direct-union-raw-stl-audit/v1',
        'native_report_sha256': sha(source_report), 'auditor_sha256': sha(Path(__file__)),
        'native_VDB_sha256': sha(vdb),
        ...
        'input_faces_removed': 0, 'source_files_modified': False, 'component_extraction_repair': False,
        ...
        'native_fields_serialized_to_VDB': True,
